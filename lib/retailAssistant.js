...
async function fetchItemsByStatus(status, limit = 20) {
  const { data } = await supabase
    .from("inventory")
    .select("rfid_tag_id, product_name, category, current_rack, updated_at")
    .eq("status", status)
...
async function buildContext() {
  ...
  const stolenLines = stolenItems.length
    ? stolenItems.map((i) => `  - ${i.product_name || "Unknown"} | Category: ${i.category || "—"} | RFID: ${i.rfid_tag_id} | Last location: ${i.current_rack || "—"}`).join("\n")
    : "  - No stolen items on record";

  const misplacedLines = misplacedItems.length
    ? misplacedItems.map((i) => `  - ${i.product_name || "Unknown"} | Category: ${i.category || "—"} | RFID: ${i.rfid_tag_id} | Last location: ${i.current_rack || "—"}`).join("\n")
