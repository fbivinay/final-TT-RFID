...
async function fetchItemsByStatus(status, limit = 20) {
  const { data } = await supabase
    .from("inventory")
    .select("rfid_tag_id, product_name, category, rack_location, last_seen")
    .eq("status", status)
...
async function buildContext() {
  const [inv, alerts, movements, stolenItems, misplacedItems] = await Promise.all([
    ...
    analyzeAlerts(),
    analyzeMovements(),
    fetchItemsByStatus("STOLEN", 20),
    fetchItemsByStatus("MISPLACED", 20),
  ]);
  ...
  const stolenLines = stolenItems.length
    ? stolenItems.map((i) => `  - ${i.product_name || "Unknown"} | Category: ${i.category || "—"} | RFID: ${i.rfid_tag_id} | Last location: ${i.rack_location || "—"}`).join("\n")
    : "  - No stolen items on record";

  const misplacedLines = misplacedItems.length
    ? misplacedItems.map((i) => `  - ${i.product_name || "Unknown"} | Category: ${i.category || "—"} | RFID: ${i.rfid_tag_id} | Last location: ${i.rack_location || "—"}`).join("\n")
