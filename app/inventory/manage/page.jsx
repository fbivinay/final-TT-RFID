  ...
  return (
    <>
      ...
      >
        <Link
          href="/inventory"
          className="inline-flex h-10 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-4 text-sm font-bold text-slate-300 hover:bg-white/[0.08]"
        >
          <ArrowLeft size={15} />
          Back to Registry
        </Link>
      ...
      {tab === 0 && (
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          <section className="panel rounded-lg p-6">
            <h2 className="mb-5 text-sm font-black uppercase tracking-[0.18em] text-white">Add New Inventory Item</h2>
            ...
            <form onSubmit={handleAdd} className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" disabled={saving} className="inline-flex h-11 items-center gap-2 rounded-lg border border-signal-cyan/40 bg-signal-cyan/16 px-5 text-sm font-bold text-signal-cyan disabled:opacity-50">
                  <Plus size={15} />
                  {saving ? "Adding..." : "Add Item"}
                </button>
                <button type="button" onClick={() => { setForm(EMPTY_FORM); setSaveMsg(null); }} className="inline-flex h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-4 text-sm font-bold text-slate-300 hover:bg-white/[0.08]">
                  <X size={15} />
                  Clear
                </button>
              </div>
          ...
          <section className="panel rounded-lg p-5">
            <h3 className="mb-4 text-xs font-black uppercase tracking-[0.18em] text-slate-400">Recently Added</h3>
              ...
              <div className="space-y-3">
                {recentAdded.map((item, i) => (
                  <div key={i} className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
                    ...
                    <p className="mt-1 font-mono text-xs text-slate-500">{item.rfid_tag_id}</p>
                    <div className="mt-2 flex items-center gap-2">
                      <StatusBadge status={item.status} />
                      <span className="text-xs text-slate-500">{item.current_rack || "—"}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {tab === 1 && (
        <section className="panel rounded-lg p-6">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-sm font-black uppercase tracking-[0.18em] text-white">Bulk CSV Import</h2>
            <button onClick={downloadTemplate} className="inline-flex h-9 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 text-xs font-bold text-slate-300 hover:bg-white/[0.08]">
              <Download size={13} />
              Download Template
            </button>
          </div>
          ...
          {!importPreview && !importResult && (
            <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-white/10 bg-white/[0.02] py-14 hover:border-signal-cyan/30 hover:bg-signal-cyan/5">
              <FileUp size={32} className="text-slate-500" />
              <div className="text-center">
                <p className="text-sm font-semibold text-slate-300">Click to upload CSV file</p>
                <p className="mt-1 text-xs text-slate-500">Columns: rfid_tag_id, product_name, category, brand, status, selling_price, mrp, etc.</p>
              </div>
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                <p className="text-sm text-slate-300">
                  <span className="font-bold text-white">{importRows.length}</span> rows found. Review before importing.
                </p>
                <button onClick={() => { setImportPreview(false); setImportRows([]); if (fileRef.current) fileRef.current.value = ""; }} className="text-xs text-slate-500 hover:text-white">
                  Clear
                </button>
              </div>

              <div className="mb-4 max-h-60 overflow-auto rounded-lg border border-white/10">
                ...
                {importRows.length > 10 && (
                  <p className="px-3 py-2 text-xs text-slate-500">…and {importRows.length - 10} more rows</p>
                )}
              </div>

              <button onClick={handleImport} disabled={importing} className="inline-flex h-11 items-center gap-2 rounded-lg border border-signal-cyan/40 bg-signal-cyan/16 px-5 text-sm font-bold text-signal-cyan disabled:opacity-50">
                <FileUp size={15} />
          ...
          {importResult && (
            <div className="space-y-3">
              {importResult.error ? (
                <div className="rounded-lg border border-signal-red/30 bg-signal-red/10 px-4 py-3 text-sm text-signal-red">{importResult.error}</div>
              ) : (
                <>
                  <div className="rounded-lg border border-signal-green/30 bg-signal-green/10 px-4 py-3 text-sm text-signal-green">
                    Successfully imported {importResult.inserted} of {importResult.total} items.
                  </div>
                  {importResult.failed.length > 0 && (
                    <div className="rounded-lg border border-signal-yellow/30 bg-signal-yellow/10 px-4 py-3">
                      <p className="mb-2 text-sm font-bold text-signal-yellow">{importResult.failed.length} rows failed validation:</p>
                  ...
                  <button onClick={() => setImportResult(null)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-4 text-sm font-bold text-slate-300">
                    Import More
                  </button>
                </>
              )}
      ...
      {tab === 2 && (
        <section className="panel rounded-lg p-6">
          <h2 className="mb-2 text-sm font-black uppercase tracking-[0.18em] text-white">Export Inventory</h2>
          ...
          {exportLoading ? (
            ...
            <>
              <div className="mb-6 rounded-lg border border-white/10 bg-white/[0.03] px-5 py-4">
                <p className="text-2xl font-bold text-white">{exportData.length.toLocaleString("en-IN")}</p>
                <p className="text-sm text-slate-400">items ready for export</p>
              </div>

              <div className="flex flex-wrap gap-3">
                <button onClick={exportCSV} disabled={!exportData.length} className="inline-flex h-11 items-center gap-2 rounded-lg border border-signal-cyan/40 bg-signal-cyan/16 px-5 text-sm font-bold text-signal-cyan disabled:opacity-40">
                  <Download size={15} />
                  Export as CSV
                </button>
                <button onClick={exportJSON} disabled={!exportData.length} className="inline-flex h-11 items-center gap-2 rounded-lg border border-signal-green/40 bg-signal-green/16 px-5 text-sm font-bold text-signal-green disabled:opacity-40">
                  <Download size={15} />
                  Export as JSON
                </button>
                <button onClick={loadExportData} className="inline-flex h-11 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-4 text-sm font-bold text-slate-300 hover:bg-white/[0.08]">
                  Refresh Count
                </button>
              </div>
              ...
              {exportData.length > 0 && (
                <div className="mt-6">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-slate-500">Preview (first 5 rows)</p>
                    ...
                    <table className="w-full min-w-[700px] text-xs">
                      ...
                      <tbody>
                        {exportData.slice(0, 5).map((row, i) => (
                          <tr key={i} className="border-t border-white/8">
                            <td className="px-3 py-2 font-mono text-slate-400">{row.rfid_tag_id}</td>
                            ...
                            <td className="px-3 py-2 text-slate-400">{row.current_rack}</td>
                            <td className="px-3 py-2"><StatusBadge status={row.status} /></td>
                            <td className="px-3 py-2 text-right text-slate-300">{currency(row.selling_price)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
