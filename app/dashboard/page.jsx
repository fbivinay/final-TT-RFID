  ...
  return (
    <>
      ...
      <div className="panel mb-6 flex items-center gap-4 rounded-lg p-5">
        <span className="flex h-12 w-12 items-center justify-center rounded-full border border-signal-cyan/30 bg-signal-cyan/10 text-signal-cyan">
          <UserCircle size={26} />
        </span>
        <div>
          <p className="font-bold text-white">{profile?.full_name || "Staff Member"}</p>
          <p className="text-sm text-slate-400">
            {department} · Status:{" "}
            <span className="font-semibold text-signal-green">{profile?.status || "ACTIVE"}</span>
          </p>
        </div>
      </div>
  ...
  return (
    <section
      className={`mb-6 rounded-lg border p-4 ${
        active ? "border-signal-green/35 bg-signal-green/10" : "border-white/10 bg-white/[0.03]"
      }`}
    >
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <span
            className={`rounded-lg border p-2 ${
              active ? "border-signal-green/40 text-signal-green" : "border-slate-600 text-slate-400"
            }`}
          >
            <RadioTower size={18} />
          </span>
        ...
        <div className="flex gap-3 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
          <span>{events.toLocaleString("en-IN")} live events</span>
          <span className={active ? "text-signal-green" : "text-slate-500"}>
            {active ? "Active" : "Idle"}
          </span>
        </div>
      </div>
